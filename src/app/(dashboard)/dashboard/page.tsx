...
export default function DashboardPage() {
  ...
  const firstName = session?.user?.name?.split(" ")[0] || "Cacciatore";
  ...
  return (
    <div className="px-4 pt-6 pb-4 max-w-md mx-auto space-y-5">
      ...
      >
        <div>
          <p className="text-slate-400 text-sm">Ciao,</p>
        ...
        <div className="flex items-center gap-2">
          <div className="glass-card px-3 py-1.5 flex items-center gap-1.5">
            ...
            <span className="text-xs text-slate-400">vittorie</span>
          </div>
        </div>
      </motion.div>

      {/* Countdown fine giornata */}
      <motion.div
      ...
      >
        ...
        <div className="flex gap-2 mb-4">
          {[0, 1, 2].map((i) => {
            ...
            return (
              <div
                key={i}
                className={`flex-1 h-14 rounded-xl border-2 flex items-center justify-center transition-all duration-300 ${
                  plate
              ...
              >
                {plate ? (
                  <div className="text-center">
                    <p className="text-xs font-mono font-bold text-brand-300 tracking-wider">
                      {plate.plateNumber}
                    </p>
                    {plate.carYear && (
                      <p className="text-xs text-slate-400">{plate.carYear}</p>
                    )}
                  </div>
                ) : (
        ...
        {(todayData?.remaining ?? 3) > 0 ? (
          <Link href="/scan" className="btn-primary w-full text-sm py-2.5">
            ...
            Scansiona targa ({todayData?.remaining ?? 3} rimaste)
          </Link>
        ) : (
          <div className="text-center py-2 text-slate-400 text-sm">
            Hai usato tutte le targhe di oggi ✓
          </div>
        )}
      </motion.div>
      ...
      {todayData?.leader && (
        ...
        >
          <div className="flex items-center gap-2 mb-3">
            <Crown
              className={`w-4 h-4 ${
                todayData.isLeading ? "text-yellow-400" : "text-slate-400"
              }`}
            />
            <h2 className="font-semibold text-white">
              {todayData.isLeading ? "Stai vincendo! 🏆" : "Leader attuale"}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="font-medium text-white text-sm">
                {todayData.leader.user.name}
              </p>
              <div className="flex items-center gap-2">
                <span className="plate-badge text-xs px-2 py-0.5">
                  {todayData.leader.plateNumber}
                </span>
                {todayData.leader.carYear && (
                  <span className="text-xs text-slate-400">
                    Anno {todayData.leader.carYear}
                  </span>
                )}
              </div>
            </div>
      ...
      >
        {[
          ...
          <div key={stat.label} className="glass-card p-3 text-center">
            <div
              className={`w-8 h-8 ${stat.bg} rounded-lg flex items-center justify-center mx-auto mb-2`}
            >
      ...
      {todayData && todayData.todayPlates.length > 0 && (
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-white">Targhe di oggi</h2>
            <Link
              href="/leaderboard"
              className="text-brand-400 text-sm flex items-center gap-1"
            >
      ...
      {!loading && todayData?.todayPlates.length === 0 && (
        ...
        >
          ...
          <p className="text-slate-400 text-sm">
            Nessuna targa ancora oggi.
            <br />
            Sii il primo a scansionare!
          </p>
